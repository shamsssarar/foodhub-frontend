      ...
      <main className="container mx-auto px-4 py-10">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-500">Master control over users and orders.</p>
          </div>
        ...
        <Tabs defaultValue="orders" className="w-full">
          <TabsList className="grid w-full grid-cols-2 mb-8 h-12">
            <TabsTrigger value="orders" className="text-md">Manage Orders</TabsTrigger>
            <TabsTrigger value="users" className="text-md">Manage Users</TabsTrigger>
          </TabsList>
                ...
                <div className="flex flex-col md:flex-row">
                  ...
                  <div className="p-6 flex-1">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1">
                          Customer: <span className="font-medium text-slate-900">{order.user?.name}</span> ({order.user?.email})
                        </p>
                    ...
                    <div className="flex items-center gap-3 pt-4 border-t border-slate-100">
                      <p className="text-xs font-bold text-slate-400 uppercase mr-auto">Actions:</p>
                      
                      {/* Status Buttons */}
                      <Button size="sm" variant="outline" onClick={() => updateOrderStatus(order.id, "PENDING")} className="h-8">Pending</Button>
                      <Button size="sm" variant="outline" onClick={() => updateOrderStatus(order.id, "IN_PROGRESS")} className="h-8 text-blue-600 border-blue-200 bg-blue-50">In Progress</Button>
                      <Button size="sm" variant="outline" onClick={() => updateOrderStatus(order.id, "DELIVERED")} className="h-8 text-green-600 border-green-200 bg-green-50">Delivered</Button>
                      <Button size="sm" variant="outline" onClick={() => updateOrderStatus(order.id, "CANCELLED")} className="h-8 text-orange-600 border-orange-200 bg-orange-50">Cancel</Button>
                ...
                <TableBody>
                  {users.map((u) => (
                    <TableRow key={u.id}>
                      ...
                      <TableCell>
                        <Badge variant="outline" className={u.role === "ADMIN" ? "bg-purple-100 text-purple-700 border-purple-200" : ""}>
                          {u.role}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <Badge className="bg-green-100 text-green-700 hover:bg-green-100 shadow-none">Active</Badge>
                      </TableCell>
